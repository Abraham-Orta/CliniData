...
// --- Mock de Notificaciones (AHORA CON REDIRECCIÓN) ---
interface AppNotification { 
  id: number; 
  type: "assignment" | "alert" | "system" | "update"; 
  title: string; 
  message: string; 
  time: string; 
  read: boolean; 
  patientId?: string; // Rastreador inyectado
}
...
const initialNotifications: AppNotification[] = [
  ...
  { id: 4, type: "system", title: "Mantenimiento", message: "Los servidores de CliniData se reiniciarán a las 03:00 AM para actualizaciones HIPAA.", time: "Ayer", read: true } // El sistema no tiene ID, no redirige
];
...
export function PatientHub({ onPatientSelect, onLogout }: { onPatientSelect: (p: Patient) => void, onLogout: () => void }) {
  ...
  const handleAddPatient = (e: React.FormEvent) => {
    ...
    setNewName(""); setNewAge(""); setNewCondition(""); setNewStatus("estable"); setNewPhone(""); setNewLocation(""); setNewBloodType("O+"); setNewAllergies("");
    setIsStatusOpen(false); setIsBloodOpen(false); setIsAddModalOpen(false);
  };
  ...
  // --- FUNCIÓN INTELIGENTE DE REDIRECCIÓN DE NOTIFICACIONES ---
  const handleNotificationClick = (notif: AppNotification) => {
    // 1. Si no está leída, la marcamos como leída
    if (!notif.read) {
      setNotifications(prev => prev.map(n => n.id === notif.id ? { ...n, read: true } : n));
    }
    
    // 2. Si la notificación tiene un paciente asociado, viajamos a su perfil
    if (notif.patientId) {
      // Buscamos en TODAS las listas (propios y compartidos)
      const targetPatient = [...localPatients, ...sharedCases].find(p => p.id === notif.patientId);
      if (targetPatient) {
        setIsNotificationsOpen(false); // Cerramos el menú desplegable
        onPatientSelect(targetPatient); // Disparamos la redirección al Frame 3
      }
    }
  };
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden" style={{ backgroundColor: "#F8FAFC", fontFamily: "Inter, sans-serif" }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <button 
                ...
                className={`relative p-2 rounded-xl transition-all ${isNotificationsOpen ? "bg-blue-50 text-[#0B5394]" : "text-slate-400 hover:bg-slate-50 hover:text-slate-700"}`}
              >
                ...
                <>
                  ...
                  <div className="absolute right-0 top-[calc(100%+8px)] w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden flex flex-col transform origin-top-right transition-all">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50/50">
                      ...
                      {unreadCount > 0 && (
                        <button onClick={markAllAsRead} className="text-xs font-semibold text-[#0B5394] hover:text-[#094074]">Marcar leídas</button>
                      )}
                    </div>
                      ...
                      {notifications.length === 0 ? (
                        ...
                        notifications.map(notif => (
                          <div 
                            ...
                            className={`p-4 border-b border-slate-50 transition-colors ${!notif.read ? "bg-blue-50/30" : ""} ${notif.patientId ? "cursor-pointer hover:bg-slate-50" : "cursor-default"}`}
                          >
          ...
          <div className="flex items-center gap-0 mb-6 border-b border-slate-200">
            {[
              ...
              <button
                ...
                style={{ fontSize: "13.5px", fontWeight: activeTab === tab.key ? 600 : 500, color: activeTab === tab.key ? "#0B5394" : "#64748b", borderBottom: activeTab === tab.key ? "2px solid #0B5394" : "2px solid transparent", marginBottom: "-1px" }}
              >
                {tab.label}
                <span className="flex items-center justify-center w-5 h-5 rounded-md" style={{ backgroundColor: activeTab === tab.key ? "#EFF6FF" : "#F1F5F9", color: activeTab === tab.key ? "#0B5394" : "#64748b", fontSize: "11px", fontWeight: 700 }}>
                  {tab.count}
                </span>
              </button>
            ))}
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto" onClick={() => { setIsAddModalOpen(false); setIsStatusOpen(false); setIsBloodOpen(false); }}>
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col my-auto" onClick={e => { e.stopPropagation(); setIsStatusOpen(false); setIsBloodOpen(false); }}>
            ...
            <form onSubmit={handleAddPatient} className="p-6 flex flex-col gap-4">
              ...
              <div className="flex gap-3 mt-2 pt-4 border-t border-slate-100">
                <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-2.5 rounded-xl font-semibold text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition-colors">Cancelar</button>
