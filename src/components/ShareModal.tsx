...
import { X, Shield, ChevronDown, CheckSquare, Square, Search, CheckCircle2, AlertTriangle } from "lucide-react";
...
export function ShareModal({ onClose, patientName, onAssign }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Ejecutamos la asignación al instante, separándola del cierre
    onAssign(assignments);
    
    // Cambiamos a la vista de éxito (lo que activará el EFECTO 2)
    setView("success");
  };
        ...
        <div className="relative w-full mx-4 rounded-2xl bg-white shadow-2xl flex flex-col max-h-[90vh]" style={{ maxWidth: "520px", fontFamily: "'Inter', sans-serif" }} onClick={(e) => e.stopPropagation()}>
          ...
          <form onSubmit={handleSubmit} className="px-7 py-6 flex flex-col gap-5 overflow-y-auto">
            ...
            <div className="flex gap-3 pt-2 border-t border-slate-100 shrink-0">
              <button type="button" onClick={handleCancelClick} className="flex-1 py-2.5 border-2 border-slate-200 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">
                Cancelar
              </button>
              <button type="submit" disabled={!isFormValid} className="flex-[2] py-2.5 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-[#0B5394] hover:bg-[#094074]">
